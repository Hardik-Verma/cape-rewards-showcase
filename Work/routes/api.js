...
router.post('/cashout', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const order = await Order.create({ user_id: user._id, name: item.name, token, points: item.cost, timestamp });
        // Manual redemption: no code is issued here. The user opens a Discord
        // ticket and our team delivers the reward manually.
...
router.all('/postback', async (req, res) => {
    ...
    const subId = params.subId || params.subid || params.user_id || params.uid || params.sub_id || params.user;
    ...
    if (!signature || signature.toLowerCase() !== expectedSig.toLowerCase()) {
        console.error(`SIGNATURE MISMATCH: subId=${subId}, transId=${transId}, reward=${reward}`);
